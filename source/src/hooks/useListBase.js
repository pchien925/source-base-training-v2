...
import { CheckOutlined, DeleteOutlined, EditOutlined, LockOutlined } from '@ant-design/icons';
import { Button, Divider, Flex, Modal, Tag } from 'antd';

import { ActionBar, BaseTooltip, SearchForm } from '@itz/react-cms-element';
import { getData, validatePermission } from '@itz/react-utils';
...
import useValidatePermission from './useValidatePermission';
...
const useListBase = ({
    ...
    const hasPermission = (requiredPermissions) => {
        return validatePermission(requiredPermissions, permissionCodes);
    };
                        ...
                        >
                            {buttons.map((ActionItem, index) => (
                                <React.Fragment key={index}>
                                    {index > 0 && <Divider style={{ top: '-0.26em' }} type="vertical" />}
                                    <span>
    ...
    const renderIdColumn = (columnsProps) => ({
     ...
     const renderStatusColumn = (columnsProps) => {
        return {
            ...
            ...columnsProps,
            render: (status) => (
                <Tag color={commonStatusColor[status]} style={{
    ...
    useEffect(() => {
        if (isCallApi || Object.keys(queryFilter).length > 0) {
            mixinFuncs.getList();
        }
        let page;
        if (tabOptions?.isTab) {
